...
function Classify(w, t, data, prediction, label0, label1, sgn)
    m0 = zeros(1,3) # points belonging to class label0
    i0 = [] # index of points
    m1 = zeros(1,3) # points belonging to class label1
    i1 = [] # index of points
...
test1_labels = ProcessData(test1_label_dir)
total_correct_test1 = sum(pred .== test1_labels)


# Test data 2
test_input_dir = "../data/testing2.txt"
pred = Classifier(test_input_dir, w, t)

# check correct
test2_label_dir = readlines("../data/testing2_label.txt")
test2_labels = ProcessData(test2_label_dir)
total_correct_test2 = sum(pred .== test2_labels)