...
from perceptron import PerceptronClassifier as Classifier1
...
class BoostingClassifier:
    ...
    def fit(self, X, y):
        ...
        for t in range(T):
            ...
            total_correct1 = sum([1 for (p,q) in zip(y_pred1,y) if p == q])
            acc1 = total_correct1/y.shape[0]
            error1 = 1 - acc1
            ...
            self.print_report(t, alpha[t], error)

        # ensemble of binary classifiers
        # self.M = sum([alpha[t]*M[t] for t in range(T)])
        self.M = lambda X: sum([alpha[t]*M[t](X) for t in range(T)])

        return self


    def print_report(self, t, alpha, error):
        incr = 1/(2*error)
        decr = 1/(2*(1-error))
        ...
        lines = [
                ...
                f"Error {error}",
                f"Alpha {alpha}",
                f"Factor to increase weights = {incr}",
                f"Factor to decrease weights = {decr}",
                ]
